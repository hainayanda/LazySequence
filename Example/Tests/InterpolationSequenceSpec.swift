...
class InterpolationSequenceSpec: QuickSpec {
    ...
    override func spec() {
        it("should do insert one element interpolated on sequence") {
            ...
            expect(array.inserted(forEach: 1, element: 0).asArray).to(equal(expected1))
            expect(array.inserted(forEach: 2, element: 0).asArray).to(equal(expected2))
            expect(array.inserted(forEach: 3, element: 0).asArray).to(equal(expected3))
        }
        it("should do insert multiple element interpolated on sequence") {
            ...
            expect(array.inserted(forEach: 1, elements: [0, 9]).asArray).to(equal(expected1))
            expect(array.inserted(forEach: 2, elements: [0, 9]).asArray).to(equal(expected2))
            expect(array.inserted(forEach: 3, elements: [0, 9]).asArray).to(equal(expected3))
        }
    }
}
