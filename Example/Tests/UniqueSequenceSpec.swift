...
class UniqueSequenceSpec: QuickSpec {
    ...
    override func spec() {
        it("should create unique sequence from projection") {
            ...
            expect(source.lazy.uniqueArray { $0.id }.compactMap { $0.id }).to(equal(expected.compactMap { $0.id }))
        }
        it("should create unique sequence from equatables") {
            ...
            expect(source.lazy.uniqueArray).to(equal(expected))
        }
        ...
        it("should create unique sequence from equatables") {
            ...
            expect(source.lazy.uniqueObjectsArray.compactMap { $0.id }).to(equal(expected.compactMap { $0.id }))
        }
    }
}
